# %% Neural Network Refinforced Learning Implementation

# Notebook with formulas and python implementation
# https://www.kaggle.com/code/wwsalmon/simple-mnist-nn-from-scratch-numpy-no-tf-keras/notebook




# %% Gradient Policy Implementation

# Good walkthrough of gradient policy with python implementation
# https://www.janisklaise.com/post/rl-policy-gradients/


# Maybe links?
# https://minpy.readthedocs.io/en/latest/tutorial/rl_policy_gradient_tutorial/rl_policy_gradient.html
# https://huggingface.co/blog/deep-rl-pg
# https://towardsdatascience.com/snake-with-policy-gradients-deep-reinforcement-learning-5e6e921db054
# https://www.sefidian.com/2021/10/01/policy-g/
# https://pylessons.com/Beyond-DQN
# https://github.com/Robin-ML/rl-policy-gradient



# %% Future Ideas

# Balancing Pole
# https://huggingface.co/blog/deep-rl-pg
